#!/usr/bin/env bash
# Own the single head-ZED instance and local JPEG exporter for letter search.
set -Eeo pipefail
...
echo "[ready] ZED serial 17064700, vision domain ${ROS_DOMAIN_ID}, local frame ${frame_file}"
wait -n "${pids[@]}"
