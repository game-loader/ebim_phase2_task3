...
def test_stage_argv_uses_configured_paths_and_interpreter():
    """Packaged runs override the interpreter, overlay and asset paths."""
    packaged = config(
        arm_root="/app",
        arm_env="/opt/ros/jazzy/setup.bash",
        dataset="/app/assets/contract",
        arm_python="/app/.venv/bin/python",
        arm_overlay="",
        config="/app/configs/tmr_rgb20d.yaml",
        calibration="/app/configs/zed_pnp_calibration.json",
        stage_poses="/app/configs/grasp_stage_poses.json",
        weights="/app/assets/weights/yolo11n-seg.pt",
        output_dir="/app/outputs",
    )
    command = build_stage_argv(packaged, "cup", "right", "abc123")[-1]
    assert "exec /app/.venv/bin/python -m" in command
    assert "--dataset /app/assets/contract" in command
    assert "--weights /app/assets/weights/yolo11n-seg.pt" in command
    assert "--calibration /app/configs/zed_pnp_calibration.json" in command
    assert "--output /app/outputs/table_stage_cup_abc123.json" in command
    # An empty overlay must not emit a bare `source ;`.
    assert "source ;" not in command
    assert "site/install/setup.bash" not in command

    # The default checkout layout still sources the colcon overlay.
    default = build_stage_argv(config(), "cup", "right", "abc123")[-1]
    assert "source site/install/setup.bash" in default
    assert "exec .venv/bin/python -m" in default
