...
class MissionConfig:
    ...
    # Paths and interpreter for the arm-local stage subprocesses. Defaults match
    # a source checkout; a packaged image overrides them.
    arm_python: str = ".venv/bin/python"
    arm_overlay: str = "site/install/setup.bash"
    config: str = "configs/tmr_rgb20d.yaml"
    calibration: str = "configs/zed_pnp_calibration.json"
...
def build_place_argv(
    ...
    arguments = " ".join(
        ...
        for value in [
            ...
            "--enable-robot",
            "--output",
            f"{config.output_dir}/table_place_{mode}_{target}_{run_id}.json",
        ]
    )
...
def _arm_python_command(config: MissionConfig, module_arguments: str) -> list[str]:
    """Run an arm-host module with the ROS overlays sourced.

    The interpreter and overlay are configurable so the same code works from a
    source checkout (``.venv/bin/python`` plus the colcon overlay under
    ``site/install``) and from a packaged image, where the interpreter lives
    elsewhere and ROS is already on the path.
    ...
    python = shlex.quote(config.arm_python)
    overlay = (
        f"source {shlex.quote(config.arm_overlay)}; " if config.arm_overlay else ""
    )
    return [
        "bash",
        "-lc",
        (
            f"set -eo pipefail; source {environment}; {overlay}"
            f"export PYTHONUNBUFFERED=1; cd {root}; "
            f'export PYTHONPATH="$PWD/src${{PYTHONPATH:+:$PYTHONPATH}}"; '
            f"exec {python} -m {module_arguments}"
        ),
    ]


def _asset_args(config: MissionConfig) -> list[str]:
    """Config, calibration, stage-pose and weight paths for a stage subprocess."""
    return [
        "--config",
        config.config,
        "--calibration",
    ...
    arguments = " ".join(
        ...
        for value in [
            ...
            *_asset_args(config),
            ...
            "--enable-robot",
            "--output",
            f"{config.output_dir}/table_stow.json",
        ]
    )
    ...
    arguments = " ".join(
        ...
        for value in [
            ...
            *_asset_args(config),
            ...
            "--enable-robot",
            "--output",
            f"{config.output_dir}/table_stage_{target}_{run_id}.json",
        ]
    )
    return _arm_python_command(config, f"franka_duo_tele_data.table_grasp_stage {arguments}")
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        help="interpreter used for the arm-local stages, relative to --arm-root or absolute",
    )
    parser.add_argument(
        "--arm-overlay",
        default="site/install/setup.bash",
        help="colcon overlay sourced before each stage; empty when ROS is already on the path",
    )
    parser.add_argument("--config", default="configs/tmr_rgb20d.yaml")
    parser.add_argument("--calibration", default="configs/zed_pnp_calibration.json")
    parser.add_argument("--stage-poses", default="configs/grasp_stage_poses.json")
    parser.add_argument("--weights", default="outputs/zed_pnp/yolo11n-seg.pt")
    parser.add_argument("--output-dir", default="outputs", help="where stage reports are written")
    ...
    config = MissionConfig(
        ...
        arm_python=args.arm_python,
        arm_overlay=args.arm_overlay,
        config=args.config,
        calibration=args.calibration,
        stage_poses=args.stage_poses,
