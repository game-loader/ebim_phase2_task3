...
- `--mode test`（假放）：下降到设定桌面高度 → 停留 → 抬起带走，**全程保持夹爪闭合**。字母侧用。
...
宽 `0.58 m`。导航时手臂必须收在这个包络内。

原有导航栈**完全不碰腰和手臂**：`tmr_base` 里 spine 只出现一次，是
